...
function onDataReceived(jsonString) {
    try {
        ...
        // 1. UPDATE GRAFIK WAVEFORM (AccZ)
        if (chartWaveform.data.datasets[0].data.length > 20) {
            chartWaveform.data.datasets[0].data.shift();
        }
        chartWaveform.data.datasets[0].data.push(accZ);
